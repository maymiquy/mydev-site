import { MarkGithubIcon, MailIcon, PersonIcon, LinkIcon, ImageIcon } from "@primer/octicons-react";
  ...
  <div className="container flex items-center justify-center min-h-screen pb-12 pt-32 px-4 mx-auto">
   <div className="grid w-full grid-cols-1 gap-8 mx-auto sm:mt-0 md:grid-cols-2 lg:grid-cols-4 lg:gap-8">
    {contancts.map((data) => {
     return (
      <Card key={data.label}>
       <Link href={data.href} target="_blank" className="p-4 relative flex flex-col items-center gap-4 duration-700 group md:gap-6 md:py-24 lg:pb-48 md:p-16 sm:p-8">
        ...
        <div className="z-10 flex flex-col items-center">
         <span className={`whitespace-nowrap text-sm font-medium duration-150 xl:text-lg text-zinc-200 group-hover:text-white font-display`}>{data.handle}</span>
         <span className="mt-4 text-sm md:text-lg text-center duration-1000 text-zinc-400 group-hover:text-zinc-200">{data.label}</span>
        </div>
       </Link>
      </Card>
     );
