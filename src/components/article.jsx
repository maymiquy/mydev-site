 ...
 if (isGitHubUser) {
  ...
  views = (
   <span title="Unique repository visitors: Last 14 days / Today.">
    <EyeIcon className="w-4 h-4" /> {Intl.NumberFormat("en-US", { notation: "compact" }).format(sumUniques)}/{Intl.NumberFormat("en-US", { notation: "compact" }).format(todayUniques)}
   </span>
  );
  ...
  alerts = (
   <span title={alertTitle}>
    <DependabotIcon className="w-4 h-4 danger" fill={alertColor} /> {Intl.NumberFormat("en-US", { notation: "compact" }).format(alertCountTotal)}
   </span>
  );
 }
  ...
  <article className="p-4 md:p-8">
   <div className="flex justify-between gap-2 items-center">
    <span className="text-xs duration-1000 text-zinc-200 group-hover:text-white group-hover:border-zinc-200 drop-shadow-orange">
     <time dateTime={new Date(project.created_at).toISOString()}>{new Date(project.created_at).toISOString().substring(0, 10)}</time>
    </span>
    <span className="text-zinc-500 text-xs flex items-center gap-1 ">
     {project.vercel && <VercelInfo info={{ ...project.vercel, owner: project.owner }} />}
     <span title="Total stars.">
      <StarIcon className="w-4 h-4" /> {Intl.NumberFormat("en-US", { notation: "compact" }).format(project.stargazers_count)}
     </span>
    </span>
   </div>
   ...
   <Link href={appLink} legacyBehavior>
    <h2 className="z-20 text-xl font-medium duration-1000 lg:text-3xl text-zinc-200 group-hover:text-white font-display cursor-pointer" title={`Click to view the ${project.homepage ? "app" : "repo"}.`}>
     <span className="bg-gradient-to-r from-zinc-100 to-zinc-100 hover:from-purple-700 hover:to-blue-700 text-transparent bg-clip-text">{project.name}</span>
    </h2>
   </Link>
